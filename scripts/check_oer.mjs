...
// --- format-contract checks (course-guide deliverable structure) ---
// Carries extra weight in LIGHT mode, where script verification substitutes for LLM cross-critique.
function formatContractAudit() {
  const typeOf = (f) => rel(f).split('/')[0];
  const fail = []; const warn = [];
  for (const f of deliverableFiles) {
    const t = typeOf(f); const text = read(f); const name = rel(f);
    if (!/^#\s+/m.test(text)) fail.push({ file: name, issue: 'missing H1 title' });
    if ((t === 'concept-maps' || t === 'assessment') && /!\[[^\]]*]\([^)]+\)/.test(text))
      fail.push({ file: name, issue: `${t} must be graphics-free but contains an image` });
    if ((t === 'concept-maps' || t === 'assessment') && /\{\{\s*(?:mermaid|mm|smiles|sm)\b/i.test(text))
      fail.push({ file: name, issue: `${t} must be graphics-free but contains a rich plugin` });
    if (t === 'slides') {
      if (!/<!--\s*LAYOUT:/i.test(text)) fail.push({ file: name, issue: 'slide spec missing <!-- LAYOUT: --> comments' });
      if (!/^---$/m.test(text)) fail.push({ file: name, issue: 'slide spec missing --- slide separators' });
      if (!/^##\s+/m.test(text)) warn.push({ file: name, issue: 'slide spec has no H2 content-slide titles' });
    }
    if (t === 'chapters' && !/!\[[^\]]+]\([^)]+\)/.test(text)) warn.push({ file: name, issue: 'study guide has no figure/image' });
