# Coursewerk — agent entry (Gemini)

Coursewerk's full entry instructions are in **[`CLAUDE.md`](CLAUDE.md)** — they are provider-neutral and
apply to any AI agent, not only Claude. **Read `CLAUDE.md` first, then `PROCEDURE.md`.** This file exists
so Gemini CLI (which auto-reads `GEMINI.md`) reaches the same canonical instructions.
...
- Before finishing, run the QA gate (`node scripts/check_oer.mjs --package package --report reports/qa_report.md`,
  which checks both Alembic-upload readiness and OER quality) until it passes, then `node scripts/pack.mjs`
  to produce the lean upload zip in `dist/`.
